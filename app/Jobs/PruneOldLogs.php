...
class PruneOldLogs implements ShouldQueue
{
    ...
    {
        $cutoffDate = now()->subDays($this->daysToKeep);
        $totalLogs = UserLoginHistory::where('created_at', '<', $cutoffDate)->count();
        ...
        $deleted = UserLoginHistory::where('created_at', '<', $cutoffDate)->delete();
    ...
    private function archiveLogs(string $logType, \DateTimeInterface $cutoffDate): void
    {
        try {
            ...
            $logs = match ($logType) {
                ...
                'login_history' => UserLoginHistory::where('created_at', '<', $cutoffDate)->get(),
